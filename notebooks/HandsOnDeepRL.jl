...
# ╔═╡ d75061ba-31d4-41e0-b0eb-c30ebd6b42c6
using Pkg

# ╔═╡ b364eba0-d400-4a2d-87ce-b61ca973c2ad
# ╠═╡ show_logs = false
pkg"add https://github.com/ancorso/Crux.jl"

# ╔═╡ 0c1b6654-6166-42eb-9c0c-803a607b8d1c
# ╠═╡ show_logs = false
pkg"add https://github.com/ancorso/POMDPGym.jl"

# ╔═╡ 12c42320-8ea4-487b-bf21-99442cbcb809
using PlutoUI

# ╔═╡ b6c897ee-a474-4b0d-beb4-629099894704
# ╠═╡ show_logs = false
using POMDPs, Crux, Flux

# ╔═╡ ce106f34-1d65-4d75-a078-9cd974f58391
using POMDPGym

# ╔═╡ f366614e-7de2-11f1-2408-cb8953867b08
md"# HandsOn Deep RL"

# ╔═╡ 55b14ff6-f8ae-487b-84c9-428f1060ab5a
md"""
## Deep RL
...
# ╔═╡ 67efbcac-19ef-4106-a191-25e58d09eaaa
solver_policy(solver) = hasproperty(solver, :π) ? solver.π : solver.agent.π

# ╔═╡ 693611fd-5456-4b90-b878-8433e56780b1
md"""
## Swinging Pendulum
...
# ╔═╡ 527f7ba3-1a65-4f33-9abf-eba63c1855e1
md"""
...
# ╔═╡ 59dbf041-3d67-4d0a-b7c8-4c033331a309
𝒜 = [-2.0, -0.5, 0.5, 2.0];

# ╔═╡ 1945112a-3fdc-491d-bed2-a5cdeb6652ef
mdp = PendulumMDP(actions=𝒜);

# ╔═╡ d1c17efd-7d02-442e-812e-85ec9489143e
𝒮 = state_space(mdp)

# ╔═╡ b7d39d3f-c5c1-4be4-8a6b-928342775b77
md"""
...
# ╔═╡ 525e2909-1d8d-4e62-a765-23c3f5820907
actor() = GaussianPolicy(action_network(), zeros(Float32, 1))

# ╔═╡ b7105752-2fcf-468c-b4cb-4d1df600e68b
md"""
### Critic
...
# ╔═╡ a32d5a28-d7cf-4978-9db5-45e3e27fda08
function critic()
	layer1 = Dense(2, 64, relu)
	layer2 = Dense(64, 64, relu)
	layer3 = Dense(64, 1)
...
# ╔═╡ a7507690-071c-4fb7-8c74-438b07ef210b
md"""
### Actor-Critic Policy
...
# ╔═╡ 2d418781-6716-4244-970b-a8038338231c
π_actor_critic = ActorCritic(actor(), critic());

# ╔═╡ 8177c0ea-905f-4c66-87c7-c3f86a532bc8
md"""
...
# ╔═╡ 8416e368-f0c3-49b8-8754-3c4ff0dd1aba
solver_ppo = PPO(π=π_actor_critic, S=𝒮, N=100000, ΔN=2048, a_opt=(batch_size=512,));

# ╔═╡ 9d928bda-8ad3-4d2f-a896-f2d8dea0d9a2
md"We use two different action spaces depending on which solution algorithm we choose. One algorithm, called _PPO_, uses a _continuous_ action space defined by a Gaussian with mean zero and unit variance (i.e., variance of one). Actions are then sampled from this policy, represented by a distribution."

# ╔═╡ 94b904b9-56ea-4117-9fe9-c58d9c823774
action_space(solver_policy(solver_ppo))

# ╔═╡ 1ee8ef69-3f93-4faf-80cf-0e0b1299a45e
md"""
...
# ╔═╡ 55df494c-69ab-4982-90a5-7b9c4fbbe111
policy_ppo = solve(solver_ppo, mdp)

# ╔═╡ c0966f4b-09a8-4425-b1b5-2c373ec3aeb3
begin
	!isdir("gifs") && mkdir("gifs")
	gif(mdp, policy_ppo, "gifs/pendulum_ppo_handson.gif", max_steps=200)
	isfile("gifs/pendulum_ppo_handson.gif") && LocalResource("./gifs/pendulum_ppo_handson.gif")
end

# ╔═╡ e9fd6977-891a-4454-9923-9beac16fde74
md"""
## Deep Q-Network (DQN)
...
# ╔═╡ 3cc7edd8-76fc-4efc-8d56-0b252f69897f
md"""
### Target network
...
# ╔═╡ 454d726b-a5a3-4067-8c75-d16af0168fd6
function Q_network()
	layer1 = Dense(2, 64, relu)
	layer2 = Dense(64, 64, relu)
...
# ╔═╡ 1bf1a0b4-ac8b-4718-9b86-764dc3bf04ed
md"""
### Solver: DQN
We construct the DQN solver using the Q-network as our policy.
"""

# ╔═╡ 21ff9079-1f8c-47c2-8e4e-33e20a8e7615
solver_dqn = DQN(π=Q_network(), S=𝒮, N=30000);

# ╔═╡ 9b6f717f-e286-40eb-be7e-4ab31bd76ec4
md"""
...
# ╔═╡ 0f943728-3be9-4ac6-907f-612c085c3c36
md"""
...
manifest_format = "2.0"
project_hash = "e8965acf09ed9481617bb8b23d23813399b4aa59"
...
git-tree-sha1 = "8fd66238d13b624bf149758177a1248d14adf844"
...
[[deps.Crux]]
deps = ["CUDA", "ColorSchemes", "Dates", "Distributions", "Flux", "Images", "LinearAlgebra", "POMDPModels", "POMDPTools", "POMDPs", "Parameters", "Plots", "Random", "Reexport", "Statistics", "StatsBase", "TensorBoardLogger", "ValueHistories", "Zygote", "cuDNN"]
git-tree-sha1 = "6c70bee4d3f53d0363b186ebe9364f5e2b63e00d"
repo-rev = "master"
repo-url = "https://github.com/ancorso/Crux.jl"
uuid = "e51cc422-768a-4345-bb8e-2246287ae729"
version = "0.1.4"

    [deps.Crux.extensions]
    WBLoggerExt = "WeightsAndBiasLogger"
...
[[deps.POMDPGym]]
deps = ["Cairo", "ColorSchemes", "Compose", "Conda", "Distributions", "Fontconfig", "GridInterpolations", "ImageCore", "ImageShow", "Images", "LinearAlgebra", "POMDPModels", "POMDPTools", "POMDPs", "Parameters", "Pkg", "PyCall", "Random", "Reexport", "StaticArrays"]
git-tree-sha1 = "9fdb732f53f43943532efbed6988107894d84a82"
repo-rev = "main"
...
git-tree-sha1 = "a87732c03138d95b58c1c70d25c92b43f4c27d3e"
...
git-tree-sha1 = "ffda2ea7ce5b247b98cce31523dcb95149aba187"
